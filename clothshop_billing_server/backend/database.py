...
database.py
------------
All SQLite access for the Cloth Shop Billing Server lives here.

This is the same schema and query logic as the original desktop app's
database.py (kept identical on purpose, so behaviour doesn't drift between
the two), with two additions for running as a shared server:

1. WAL journal mode -- lets one write happen while other requests are
   still reading, which matters now that several phones/laptops can hit
   the database at the same time over the LAN.
2. Automatic backups -- every method that writes (insert/update/delete)
   opens its connection with write=True. On successful commit, _conn()
   copies the live .db file into data/backups/ with a timestamp in the
   name, then prunes old backups beyond MAX_BACKUPS. If anything raises
   inside the `with` block, the exception propagates before the backup
   line is ever reached, so a failed write is never backed up.
"""
...
MAX_BACKUPS = 200  # keep the last N backup copies, prune older ones


def _default_db_path() -> str:
    """Store the DB inside backend/data, next to this file, so the whole
    server folder is self-contained and easy to back up / move as a unit."""
...
# Purely a starting point -- fully editable / deletable from the Inventory
# tab afterwards. Not referenced anywhere else in code.
...
class Database:
    ...
    def _backup_db(self):
        try:
            ...
            dest = os.path.join(self.backup_dir, f"cloth_shop_{ts}.db")
            shutil.copy2(self.path, dest)
            self._prune_backups()
        except Exception:
            # A backup failure should never take the server down or block
            # the write that already succeeded -- just log it.
            log.exception("Backup failed")

    def _prune_backups(self):
        files = sorted(glob.glob(os.path.join(self.backup_dir, "cloth_shop_*.db")))
    ...
    def _conn(self, write: bool = False):
        ...
        try:
            ...
            conn.close()
        # Only reached if the `with` block above didn't raise.
        if write:
            self._backup_db()
    ...
    def get_subtypes(self, category_id: int):
        with self._conn() as conn:
            return conn.execute(
                "SELECT * FROM subtypes WHERE category_id=? ORDER BY name", (category_id,)
            ).fetchall()
    ...
    def rename_subtype(self, subtype_id: int, new_name: str):
        with self._conn(write=True) as conn:
            conn.execute("UPDATE subtypes SET name=? WHERE id=?", (new_name.strip(), subtype_id))
    ...
    def adjust_stock(self, item_id, delta):
        with self._conn(write=True) as conn:
            conn.execute("UPDATE items SET stock_qty = stock_qty + ? WHERE id=?", (delta, item_id))
    ...
    def get_customer_by_phone(self, phone):
        with self._conn() as conn:
            return conn.execute("SELECT * FROM customers WHERE phone=?", (phone.strip(),)).fetchone()
    ...
    def get_customer_purchase_history(self, cid):
        with self._conn() as conn:
            return conn.execute(
                "SELECT * FROM bills WHERE customer_id=? ORDER BY bill_date DESC", (cid,)
            ).fetchall()
    ...
    def set_wishlist_fulfilled(self, wishlist_id, fulfilled=1):
        with self._conn(write=True) as conn:
            conn.execute("UPDATE customer_wishlist SET fulfilled=? WHERE id=?", (fulfilled, wishlist_id))
