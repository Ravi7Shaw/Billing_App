...
customers_tab.py
-----------------
Search customers, see their purchase history, and manage what they said
they want next time -- so staff can look it up the moment the customer
walks back in.
"""
...
class CustomersTab(QWidget):
    ...
    def _build_ui(self):
        ...
        outer.setSpacing(12)
        outer.addWidget(make_heading("Customers", "Search by name or phone, view history, track what they want next"))
        ...
        self.customer_table.setHorizontalHeaderLabels(["Name", "Phone", "Spent"])
        ...
        remove_customer_btn.setProperty("role", "danger")
        remove_customer_btn.clicked.connect(self._delete_customer)
        save_row.addWidget(remove_customer_btn)
        save_btn = QPushButton("Save Changes")
        save_btn.clicked.connect(self._save_customer_details)
        ...
        self.history_table.setHorizontalHeaderLabels(["Bill No", "Date", "Items", "Total", ""])
        ...
        right_v.addWidget(history_box, 1)
        ...
        self.wishlist_table.setHorizontalHeaderLabels(["Wants", "Added", ""])
        self.wishlist_table.horizontalHeader().setSectionResizeMode(0, QHeaderView.Stretch)
        self.wishlist_table.horizontalHeader().setSectionResizeMode(2, QHeaderView.Fixed)
        self.wishlist_table.setColumnWidth(2, 90)
        self.wishlist_table.setEditTriggers(QTableWidget.NoEditTriggers)
        wishlist_v.addWidget(self.wishlist_table)

        right_v.addWidget(wishlist_box, 1)
    ...
    def _refresh_customer_list(self):
        ...
        for row_idx, c in enumerate(customers):
            ...
            self.customer_table.setItem(row_idx, 0, QTableWidgetItem(c["name"]))
            self.customer_table.setItem(row_idx, 1, QTableWidgetItem(c["phone"] or "-"))
            self.customer_table.setItem(row_idx, 2, QTableWidgetItem(rupees(total_spent)))
    ...
    def _load_customer(self, cid):
        ...
        self.summary_label.setText(
            f"Customer since {customer['created_at'][:10]} \u2022 {visit_count} visit(s) \u2022 {rupees(total_spent)} total spent"
        )

        history = self.db.get_customer_purchase_history(cid)
        self.history_table.setRowCount(len(history))
        for row_idx, b in enumerate(history):
            self.history_table.setItem(row_idx, 0, QTableWidgetItem(b["bill_no"]))
            self.history_table.setItem(row_idx, 1, QTableWidgetItem(b["bill_date"]))
            _, items = self.db.get_bill(b["id"])
            piece_count = sum(i["quantity"] for i in items)
            self.history_table.setItem(row_idx, 2, QTableWidgetItem(str(piece_count)))
            self.history_table.setItem(row_idx, 3, QTableWidgetItem(rupees(b["total"])))
            ...
            self.history_table.setCellWidget(row_idx, 4, remove_btn)
    ...
    def _refresh_wishlist(self, cid):
        ...
        for row_idx, w in enumerate(wishlist):
            ...
            self.wishlist_table.setItem(row_idx, 0, desc_item)
            self.wishlist_table.setItem(row_idx, 1, QTableWidgetItem(w["date_added"][:10]))
            ...
            btn.clicked.connect(lambda _, i=wid, s=new_state: self._toggle_wishlist(i, s))
            self.wishlist_table.setCellWidget(row_idx, 2, btn)
        self.wishlist_table.resizeRowsToContents()

    def _toggle_wishlist(self, wishlist_id, state):
        self.db.set_wishlist_fulfilled(wishlist_id, state)
    ...
    def _save_customer_details(self):
        if not self.selected_customer_id:
            QMessageBox.information(self, "No customer selected", "Select a customer from the list first.")
            return
        name = self.name_input.text().strip()
        if not name:
            QMessageBox.warning(self, "Missing info", "Name cannot be empty.")
            return
        self.db.update_customer(
            self.selected_customer_id, name, self.phone_input.text().strip(),
            self.address_input.text().strip(), ""
        )
        self._refresh_customer_list()
        QMessageBox.information(self, "Saved", "Customer details updated.")
    ...
    def _add_wishlist(self):
        ...
        if not text:
            return
        self.db.add_wishlist(self.selected_customer_id, text)
    ...
    def _delete_customer(self):
        ...
        confirm = QMessageBox.question(
            ...
            f"Remove {name} from your customer list? Their past bills stay in your sales records "
            f"as walk-in sales, but their profile and wishlist will be deleted."
        )
        if confirm != QMessageBox.Yes:
            return
        if not confirm_delete_password(self, f"remove {name}"):
            return
